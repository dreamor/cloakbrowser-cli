...
Sign up for CloakBrowser via [my referral link](https://www.orcarouter.ai/ref/ref_832a43c568e048ba012b) — costs you nothing extra, helps keep this CLI maintained.
---
...
Requires Node 20.0+. Peer deps `cloakbrowser` (≥ 0.3.0, 0.5.10+ recommended) and `playwright-core` (≥ 1.53.0) are installed alongside. Optional peers `mmdb-lib` and `socks-proxy-agent` enable Pro GeoIP and SOCKS proxy features.
...
- **Context-level**: `--storage-state <path>`, `--extra-headers <json>`, `--permissions <json>`, `--extension <path>` (⚠️ currently non-functional: Chromium 137+ removed `--load-extension` side-loading — CLI fails fast with UNSUPPORTED_OPERATION)
